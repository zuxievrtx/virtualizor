...
function virtualizor_CreateAccount($params) {
	...
	if(!empty($ret['newvs']['vpsid'])){
		...
		Capsule::table('tblhosting')
			->where('id',$serviceid)
			->update(
				array('dedicatedip' => (!empty($_ips[0]) ? $_ips[0] : (!empty($_ips6[0]) ? $_ips6[0] : (!empty($_ips6_subnet[0]) ? $_ips6_subnet[0] : $ret['newvs']['ips_int'][0]))))
			);
...
function virtualizor_AdminLink($params) {
	...
	$redirect_url = 'https://'.$serverip.':4085/index.php?act=login';
	if(!empty($virtualizor_conf['enable_admin_sso'])){
		...
		$redirect_url = 'https://'.$serverip.':4085/'.$ret['token_key'].'/?as='.$ret['sid'].'&goto_cp='.rawurlencode(virtualizor_get_current_url());
...
function virtualizor_newUI($params, $url_prefix = 'clientarea.php?action=productdetails', $modules_url = 'modules/servers'){
	...
	if(isset($_GET['give'])){
		...
		if($_GET['give'] == 'combined.js'){
			...
			$data .= 'var disabletoggletheme = '.$virtualizor_conf['disable_toggle_theme'].';';
...
var start_avail = document.getElementById("Primary_Sidebar-Service_Details_Actions-Custom_Module_Button_Start_VPS");
if (start_avail) {
...
var stop_avail = document.getElementById("Primary_Sidebar-Service_Details_Actions-Custom_Module_Button_Stop_VPS");
if (stop_avail) {
...
var poweroff_avail = document.getElementById("Primary_Sidebar-Service_Details_Actions-Custom_Module_Button_Poweroff_VPS");
if (poweroff_avail) {
...
var reboot_avail = document.getElementById("Primary_Sidebar-Service_Details_Actions-Custom_Module_Button_Reboot_VPS");
if (reboot_avail) {
