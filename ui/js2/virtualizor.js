...
function checktheme(page){
	...
	if((typeof disabletoggletheme !== 'undefined') && (disabletoggletheme > 0)){
		document.getElementById("toggletheme").classList.add("hidden");
		return;
	}
	...
	//console.log(page);
	// icons
	const sunIcon = document.querySelector("."+page+"virtualizor-light");
	const moonIcon = document.querySelector("."+page+"virtualizor-dark");
...
function show_cpinstall_window(){
	...
	if(!empty(N['custom_cp'])){
		$.each(N['custom_cp'], function(i,v){
			$panels_avail.push(v['name']);
			var img = '[[images]]default_custom_control.png';
			if(!empty(v['img'])){
				img = '[[images]]'+i+'.png';
			}
			var newDivHTML = '<div class="cplogos" id="'+v['name']+'" style="display: none;"><input class="w-12" type="image" name="'+i+'" src="'+img+'" nsrc="'+img+'" /><h4 class=" font-semibold">'+v['name']+'</h4></div>';

			$('#installcp .cptab').append(newDivHTML);
		});
	}
	
	if(!empty(N['info']['flags']['disable_icons_cp_array'])){
		$.each($panels_avail, function(i, v) {
			if (Object.values(N['info']['flags']['disable_icons_cp_array']).indexOf('disable_icons_cp_'+v) == -1) {
				$('#'+v).show();
			}
		});
	}else{
			...
			table += `<tr data-node-id="`+date+`" data-node-pid="`+date+`">
						...
						<td class="text-center">
							<div class="flex gap-3">
								<a href="javascript:void(0);" title="Restore" onclick="backuply_restore_bkup('`+files['htime']+`', '`+date+`')"><i class="fas fa-history text-blue-600"></i></a>
								<a href="javascript:void(0);" title="Delete" onclick="backuply_delete_bkup('`+files['htime']+`', '`+date+`', '`+files['type']+`')"><i class="fas fa-trash danger"></i></a>
							</div>
						</td>
					</tr>`;
			if(!empty(files['children'])){
				$.each(files['children'], (k, file) => {
					...
					table += `<tr data-node-pid="`+date+`">
								...
								<td class="text-center">
									<div class="flex gap-3">
										<a href="javascript:void(0);" title="Restore" onclick="backuply_restore_bkup('`+file['htime']+`', '`+date+`')"><i class="fas fa-history text-blue-600"></i></a>
										<a href="javascript:void(0);" title="Delete" onclick="backuply_delete_bkup('`+file['htime']+`', '`+date+`', '`+files['type']+`')"><i class="fas fa-trash danger"></i></a>
									</div>
								</td>
							</tr>`;
				})
...
function set_backups_table(backups_list, restore) {
	...
	$.each(backups_list, function(date, value){
		...
		$.each(value, function(fk, file){
			...
			var action_restore = '<span class="flex gap-3">'+(restore ? '<a href="javascript:void(0)" title="{{bkup2_restore}}" onclick="restore(' + date + ', \''+file+'\'); return false;"><i class="fas fa-history text-blue-600 mr-2"></i>' : '');

			table_data.push(['<span title = "'+file+'">'+_date+'</span>' ,action_restore + '<a href="javascript:void(0)" title="{{delete}}" onclick="backup_delete(' + date + ', \''+file+'\'); return false;"><i class="far fa-1x fa-trash-alt danger" aria-hidden="true"></i></a></span>']);
		})
	})
		...
		if (!$.fn.DataTable.isDataTable( "#managerules")) {
			...
			$("#managerules").DataTable({
				...
				"columnDefs": [
					...
					{ className: "width-15", "targets": [ 5 ] },
					{ className: "width-20", "targets": [ 6 ] },
					{ className: "width-9", "targets": [ 7 ] }
				],
...
function addfirewallrule(is_edit = 0){
	...
	if(port == "" && !(protocol_val == "icmp" || protocol_val == "gre" || protocol_val == "esp") ){
		error_alert("{{fw_empty_port}}");
		return false;
	}
	...
	AJAX('[[API]]act=firewallplan&get_firewall_plan='+fw, function(data) {
		...
		if(!empty(template_rules)){
			...
			template_rules.forEach(function(v){
				...
				protocol = tmp_cell[3];
				port = tmp_cell[4];
				src_address = tmp_cell[5];

				firewalllist_table.row.add( 
					["", ip_protocol, direction, policy, protocol, port, src_address]
				).draw();
			});
...
function edit_firewallplan_tab(fwpid){
	...
	AJAX('[[API]]act=editfirewallplan&fwpid='+fwpid, function(data) {
		...
		if(!empty(N["firewall_plans"]["data"])){

			let firewall_metadata = N["firewall_plans"]["data"];

			if(!empty(firewall_metadata["note"])){
				$('#edit_fwp_note').val(firewall_metadata["note"]);
			}
		...
		firewall_rules.forEach(function(v){
			...
			let port_val = tmp_rule[4];
			let src_addr = tmp_rule[5];
			...
			firewallplan_table.row.add(
				...
				"<input type=\"hidden\" name=\"decision[]\" value=\""+decision_val+"\" />"+decision_display,
				"<input type=\"hidden\" name=\"protocol[]\" value=\""+protocol_val+"\" />"+protocol_display, 
				"<input type=\"hidden\" name=\"port[]\" value=\""+port_val+"\" />"+port_val,
				"<input type=\"hidden\" name=\"source_addr[]\" value=\""+src_addr+"\" />"+src_addr, 
